...
**Formula:** Given pairs \\(X_i, Y_i)\\, sort them such that \\X\_{(1)}
\le \dots \le X\_{(n)}\\. Let \\r_i\\ be the rank of \\Y\_{(i)}\\. The
coefficient is: \$\$ \xi_n(X, Y) = 1 - \frac{3 \sum\_{i=1}^{n-1}
\|r\_{i+1} - r_i\|}{n^2 - 1} \$\$
