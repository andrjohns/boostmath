...
#'
#' * `acosh_boost(x)`: Computes arccosh(x) = cosh⁻¹(x)
#' * Mathematical definition: acosh(x) = log(x + sqrt(x² - 1))
#' * Domain: x ≥ 1
#' * Range: [0, ∞)
#' * Identity: cosh(acosh(x)) = x for x ≥ 1
#' * Derivative: d/dx acosh(x) = 1/sqrt(x² - 1)
#'
#' **Inverse Hyperbolic Sine:**
#'
#' * `asinh_boost(x)`: Computes arcsinh(x) = sinh⁻¹(x)
#' * Mathematical definition: asinh(x) = log(x + sqrt(x² + 1))
#' * Domain: (-∞, ∞)
#' * Range: (-∞, ∞)
#' * Odd function: asinh(-x) = -asinh(x)
#' * Identity: sinh(asinh(x)) = x for all x
#' * Derivative: d/dx asinh(x) = 1/sqrt(x² + 1)
#'
#' **Inverse Hyperbolic Tangent:**
#'
#' * `atanh_boost(x)`: Computes arctanh(x) = tanh⁻¹(x)
#' * Mathematical definition: atanh(x) = (1/2)log((1 + x)/(1 - x))
#' * Domain: (-1, 1)
#' * Range: (-∞, ∞)
#' * Odd function: atanh(-x) = -atanh(x)
#' * Identity: tanh(atanh(x)) = x for |x| < 1
#' * Derivative: d/dx atanh(x) = 1/(1 - x²)
#' * Special values: atanh(0) = 0, atanh(±1) = ±∞
#'
#' **Applications:**
#'
#' These functions appear in:
#' * Special relativity (rapidity transformations)
#' * Integration (e.g., ∫ 1/sqrt(x² + 1) dx = asinh(x) + C)
